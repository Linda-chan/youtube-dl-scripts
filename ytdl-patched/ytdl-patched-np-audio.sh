...
if [[ -e "$MY_DP/ytdl-patched.cfg" ]] ; then
  ytdl-patched --proxy "" --format bestaudio/best --extract-audio $PARAMS --config-location "$MY_DP/ytdl-patched.cfg" "$@"
else
  ytdl-patched --proxy "" --format bestaudio/best --extract-audio $PARAMS "$@"
fi
