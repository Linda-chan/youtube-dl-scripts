...
if [[ -e "$MY_DP/youtube-dl.cfg" ]] ; then
  youtube-dl --proxy "" --format bestaudio/best --extract-audio $PARAMS --config-location "$MY_DP/youtube-dl.cfg" "$@"
else
  youtube-dl --proxy "" --format bestaudio/best --extract-audio $PARAMS "$@"
fi
